...
export default function DriverTripLocationPanel({ onReadyChange, refreshToken = 0 }: { onReadyChange?: (ready: boolean) => void; refreshToken?: number }) {
  ...
  return (
    <div className="mx-auto max-w-screen-lg px-4 pt-3 sm:px-6">
      <div className={`rounded-2xl border p-4 card-shadow-sm ${tracking || ready ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="section-label">Trip location</p>
              <p className={`mt-0.5 text-sm font-extrabold ${tracking || ready ? 'text-green-900' : 'text-amber-900'}`}>
                {tracking ? 'Location sharing active' : ready ? 'Location ready' : 'Location needed before departure'}
              </p>
              <p className={`mt-1 text-xs leading-relaxed ${tracking || ready ? 'text-green-800' : 'text-amber-800'}`}>
                {tracking
          ...
          {!tracking && (
            <button type="button" onClick={enableLocation} disabled={busy} className="btn-primary w-full shrink-0 sm:w-auto sm:px-5">
