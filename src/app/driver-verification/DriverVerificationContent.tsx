...
import { CheckCircle2, FileCheck2, FileText, ImageIcon, Loader2, RefreshCw, ShieldAlert, ShieldCheck, Trash2, Upload } from 'lucide-react';
...
type Verification={
  ...
  driving_licence_status:VerifyStatus;vehicle_rc_status:VerifyStatus;car_photos_status:VerifyStatus;
  driving_licence_notes?:string|null;vehicle_rc_notes?:string|null;car_photos_notes?:string|null;
  documents:DriverDoc[];
};
...
const label:Record<DocType,string>={DRIVING_LICENCE:'Driving Licence',VEHICLE_RC:'Vehicle RC',CAR_PHOTO:'Car photos'};
...
export default function DriverVerificationContent(){
  ...
  const upload=async(type:DocType,file:File)=>{
    ...
    if(type==='CAR_PHOTO'&&file.type==='application/pdf') return toast.error('Car photos must be images');
    ...
    const {data,error}=await supabase.rpc('register_driver_verification_upload',{
      p_document_type:type,p_storage_path:path,p_original_name:file.name,p_mime_type:file.type,p_file_size:file.size,
    });
    setBusy(null);
  ...
  const choose=(type:DocType)=>(event:ChangeEvent<HTMLInputElement>)=>{
    const file=event.target.files?.[0]; event.target.value=''; if(file) void upload(type,file);
  };

  const retire=async(doc:DriverDoc)=>{
    if(!window.confirm(`Remove ${doc.original_name} from current verification?`)) return;
    setBusy(`retire-${doc.document_id}`);
    const supabase=createClient();
    ...
    const {error:removeError}=await supabase.storage.from('driver-verification').remove([doc.storage_path]);
    setBusy(null);
  ...
  const fully=state.driving_licence_status==='VERIFIED'&&state.vehicle_rc_status==='VERIFIED'&&state.car_photos_status==='VERIFIED';
  ...
  return <div className="mx-auto max-w-screen-lg space-y-5 px-4 py-5 sm:px-6 sm:py-8">
    ...
    <section className="feature-card p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3"><div><p className="section-label">Vehicle on Raahi</p><p className="mt-1 text-base font-extrabold">{state.vehicle?.vehicle_model||state.vehicle?.vehicle_type||'Vehicle'}</p><p className="mt-1 text-xs text-muted-foreground">{state.vehicle?.registration_number||'Registration unavailable'}{state.vehicle?.capacity?` · ${state.vehicle.capacity} seats`:''}</p></div><Link href="/driver-route-selection" className="btn-outline px-3 py-2">Driver Home</Link></div>
    </section>
...
function VerificationCard({type,status,notes,docs,busy,onChoose,onRetire}:{type:DocType;status:VerifyStatus;notes?:string|null;docs:DriverDoc[];busy:string|null;onChoose:(e:ChangeEvent<HTMLInputElement>)=>void;onRetire:(doc:DriverDoc)=>void}){
  const car=type==='CAR_PHOTO';
  const accept=car?'image/jpeg,image/png,image/webp':'image/jpeg,image/png,image/webp,application/pdf';
  const canAdd=!car||docs.length<4;
  return <section className="feature-card p-4 sm:p-5">
    <div className="flex items-start justify-between gap-3">
      <div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-secondary text-primary">{car?<ImageIcon size={19}/>:<FileText size={19}/>}</div><div><h2 className="text-sm font-extrabold">{label[type]}</h2><span className={`mt-1 inline-block rounded-full px-2.5 py-1 text-[10px] font-bold ${statusClass[status]}`}>{statusCopy[status]}</span></div></div>
      <label className={`btn-outline cursor-pointer px-3 py-2 ${!canAdd?'pointer-events-none opacity-40':''}`}><Upload size={14}/>{docs.length?'Add / replace':'Upload'}<input type="file" accept={accept} className="hidden" disabled={!canAdd||busy===`upload-${type}`} onChange={onChoose}/></label>
    </div>
    <p className="mt-3 text-xs leading-relaxed text-muted-foreground">{car?'Add up to 4 clear exterior/interior photos. Adding or removing a photo returns this group to review.':'JPG, PNG, WebP or PDF · up to 8 MB. Replacing a document returns it to review.'}</p>
...
function DocumentRow({doc,busy,onRetire}:{doc:DriverDoc;busy:boolean;onRetire:()=>void}){
  const size=doc.file_size<1024*1024?`${Math.max(1,Math.round(doc.file_size/1024))} KB`:`${(doc.file_size/1024/1024).toFixed(1)} MB`;
  return <div className="flex items-center gap-3 rounded-2xl border border-border px-3 py-3"><FileCheck2 size={16} className="shrink-0 text-primary"/><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{doc.original_name}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{size} · uploaded {new Date(doc.created_at).toLocaleDateString()}</p></div><button onClick={onRetire} disabled={busy} className="flex h-9 w-9 items-center justify-center rounded-xl border border-border text-red-600" aria-label={`Remove ${doc.original_name}`}>{busy?<Loader2 size={14} className="animate-spin"/>:<Trash2 size={14}/>}</button></div>;
}

function Access({title,text}:{title:string;text:string}){
  return <div className="mx-auto max-w-screen-sm px-4 py-16 text-center"><ShieldAlert size={36} className="mx-auto text-muted-foreground opacity-40"/><h1 className="mt-4 text-lg font-bold">{title}</h1><p className="mt-2 text-sm text-muted-foreground">{text}</p></div>;
}
