...
function ObserverSurface({ role, stage }: { role: Role; stage: number }) {
  let title='Marketplace state', detail='Raahi is preparing the next state.', tone:Tone='quiet'; let rows:string[]=[];
  if(role==='admin') {
    if(stage===0){title='No operating area is live';detail='Passenger and Driver entry points can exist without pretending supply is available.';rows=['0 published corridors','0 active Outstation origins','0 pending Driver applications'];}
    else if(stage<5){title='Pilot network is published';detail=stage>=4?'Rajeev4 is filling his own application. Nothing enters the Admin queue until he submits.':'Drivers can now self-join while Passenger discovery shows only published services.';rows=['2 Shared Ride corridors','3 Outstation origin areas',stage>=4?'0 applications submitted':'Driver self-onboarding open'];}
    else if(stage===5){title='1 Driver application needs review';detail='Ajit sees the documents Rajeev4 submitted and decides approve, reject or request replacement.';tone='warn';rows=['Rajeev4 · Bokaro','DL + RC + car photos uploaded','Operations still locked'];}
    else if(stage<8){title='Verified supply is growing';detail='Rajeev4 is approved for Bokaro Outstation. Shared Ride and Outstation preferences remain independent.';tone='good';rows=['Rajeev4 · Bokaro verified','2 Shared Ride corridors published','No open Outstation request'];}
    else if(stage===8){title='Shared Ride demand is active';detail='Admin can see the corridor is alive without choosing the Driver order manually.';tone='good';rows=['Dhanbad → Gomoh','Naresh · ACTIVE_COLLECTING','2 seats held · 2 remain'];}
    else if(stage===9){title='Passenger is planning flexible travel';detail='No Outstation lead exists until the Passenger actually submits the request.';rows=['Bokaro origin active','0 open Bokaro requests','Verified supply ready'];}
    else if(stage===10){title='1 Bokaro Outstation request is open';detail='Only eligible Bokaro-origin Drivers see the lead.';tone='warn';rows=['Bokaro → Ranchi → Bokaro','4 Passengers','Awaiting Driver quotes'];}
    else if(stage===11){title='Quotes are arriving';detail='Admin sees response health, while the Passenger—not Admin—chooses the Driver.';tone='good';rows=['2 Driver responses','₹3,800 best current quote','Contact still private'];}
    else if(stage<14){title='Outstation booking accepted';detail=stage===12?'Rajeev4 won the booking and the operating relationship is unlocked.':'Trip completed; marketplace can return Driver supply to availability.';tone='good';rows=['Passenger selected Rajeev4','Contact unlocked after acceptance','Launch controls unchanged'];}
    else {title='Demand is becoming market intelligence';detail='Repeated requests can suggest the next corridor without automatically creating one.';tone='good';rows=['18 Bokaro → Ranchi asks','Corridor candidate detected','Admin still decides'];}
  } else if(role==='driver') {
    if(stage<2){title='Drive with Raahi is available';detail='Drivers can discover the opportunity without Admin creating an account for them.';rows=['Google sign-in','Phone OTP','Self-service application'];}
    else if(stage===5){title='Application submitted';detail='Rajeev4 waits for Admin review. Paid Driver actions remain locked.';tone='warn';rows=['DL uploaded','RC uploaded','Car photos uploaded'];}
    else if(stage<9){title='Rajeev4 is verified for Bokaro';detail='He can receive Bokaro Outstation leads. Shared Ride requires a separate corridor choice.';tone='good';rows=['Outstation · Bokaro ON','Shared Ride preferences separate','No Outstation lead yet'];}
    else if(stage===9){title='Ready for Bokaro leads';detail='The Passenger has not submitted the quote request yet, so Drivers see nothing new.';rows=['Bokaro subscription ON','Verified vehicle ready','No open lead'];}
    else if(stage===11){title='₹3,800 quote sent';detail='Rajeev4 waits. He does not get Passenger contact details before acceptance.';tone='warn';rows=['Bokaro → Ranchi → Bokaro','Quote · ₹3,800','Awaiting Passenger decision'];}
    else if(stage>=13){title='Trip complete · Driver available again';detail='The completed trip does not silently change Shared Ride or Outstation subscriptions.';tone='good';rows=['Booking closed','Bokaro subscription remains ON','Ready for next eligible lead'];}
    else {title='Driver marketplace state';detail='Driver eligibility follows verification plus explicit service preferences.';rows=['Verified Driver','Bokaro origin preference','No unrelated notifications'];}
  } else {
    if(stage===0){title='Raahi is not pretending service exists';detail='Passenger discovery stays honest until Admin publishes an operating network.';rows=['No bookable corridor','No Outstation origin advertised','Try again when Raahi launches locally'];}
    else if(stage<6){title='Published travel is visible';detail='Passengers can plan around the fixed corridors and active Outstation origins while Driver supply is being built.';rows=['Dhanbad ⇄ Gomoh · Shared Ride','Parasnath → Madhuban · Shared Ride','Bokaro · Outstation origin'];}
    else if(stage<8){title='Bokaro now has verified Driver supply';detail='Passenger still enters only From + To; Raahi decides which marketplace to use.';tone='good';rows=['Shared Ride when corridor matches','Outstation when origin is active','No ride-mode expertise required'];}
    else if(stage===8){title='2 Shared Ride seats are held';detail='Passenger sees the active FIFO car and exact remaining seat capacity.';tone='good';rows=['Naresh · active Driver','DL + RC verified','2 seats remain'];}
    else if(stage===10){title='Request sent · waiting for quotes';detail='Passenger does not see private Driver contact details while the request is open.';tone='warn';rows=['Bokaro → Ranchi → Bokaro','Round trip','0 accepted Drivers yet'];}
    else if(stage===12){title='Rajeev4 is confirmed';detail='Contact and trip coordination unlock only after Passenger acceptance.';tone='good';rows=['₹3,800 accepted','Driver + car trust card retained','Contact now available'];}
    else if(stage>=14){title='Raahi quietly learns from demand';detail='Passenger keeps a simple travel experience while Admin sees corridor opportunity signals.';tone='good';rows=['No extra Passenger complexity','Future corridor may improve pricing/density','Existing booking history unchanged'];}
    else {title='Passenger marketplace state';detail='Raahi keeps the next travel action clear and hides internal marketplace complexity.';rows=['One From + To search','Verified supply only','Privacy until acceptance'];}
  }
  return <PhoneFrame role={role} title="Live marketplace reaction"><div className="flex items-start justify-between gap-3"><div><p className="section-label">What this role sees now</p><h2 className="mt-1 text-lg font-extrabold">{title}</h2></div><Pill tone={tone}>{tone==='good'?'LIVE':tone==='warn'?'WAITING':'STATE'}</Pill></div><p className="mt-2 text-xs leading-relaxed text-muted-foreground">{detail}</p><div className="mt-4 space-y-2">{rows.map(row=><div key={row} className="flex items-center gap-2 rounded-xl border border-border bg-white p-3"><CheckCircle2 size={14} className="shrink-0 text-primary"/><p className="text-xs font-semibold">{row}</p></div>)}</div><div className="mt-4 rounded-xl bg-muted p-3 text-[10px] leading-relaxed text-muted-foreground">Switch back to the highlighted action role to perform the next marketplace action. This view changes the screen, not the story state.</div></PhoneFrame>;
}
...
function renderSurface(stage: number, role: Role, advance: () => void) {
  if (stage === 8 && role === 'passenger') return <SharedRideLive role="passenger" advance={advance}/>;
  if (role !== STORY[stage].actor) return <ObserverSurface role={role} stage={stage}/>;
