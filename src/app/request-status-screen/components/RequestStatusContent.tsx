...
export default function RequestStatusContent() {
  ...
  return (
    <div className="mobile-page space-y-3 animate-fade-in">
      <UnifiedTripCard from={routeFrom} to={routeTo} statusLabel={liveStatusLabel} statusTone={liveStatusTone} vehicleLabel={`${req.driver_display_name} · ${req.vehicle_number}`} pickupLabel={req.pickup_stop_name} confidenceLabel={pickupProgressText}>
      ...
      {isConfirmed && <div className="flex items-start gap-3 bg-green-50 border border-green-200 rounded-2xl px-4 py-3"><CheckCircle2 size={20} className="text-green-600 flex-shrink-0 mt-0.5" /><div><p className="text-sm font-bold text-green-800">{isTripCompleted ? 'Trip Completed' : 'Booking Confirmed'}</p><p className="text-xs text-green-700 mt-0.5">{isTripCompleted ? 'You have arrived at the destination. Thank you for riding with Raahi!' : `${seatLabel} confirmed. The driver has received your payment. Enjoy your ride!`}</p></div></div>}
