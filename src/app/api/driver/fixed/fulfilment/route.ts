...
export async function POST(request: Request) {
  ...
  if (result.error) {
    ...
    if (hit) {
      if (["ARRIVAL_LOCATION_NOT_VERIFIED","COMPLETION_LOCATION_NOT_VERIFIED","ROUND_TRIP_LOCATION_NOT_VERIFIED"].includes(hit[0]) && input.rideId && input.accuracyMeters != null && input.capturedAt) {
