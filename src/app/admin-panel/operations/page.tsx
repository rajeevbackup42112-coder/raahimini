...
import { AlertTriangle, ArrowDown, ArrowUp, Car, Gauge, Loader2, MapPin, RefreshCw, ShieldAlert, ShieldCheck, Trash2, UserX, Users } from 'lucide-react';
...
export default function AdminOperationsPage() {
  ...
  const openSupport=summary?.open_support_cases ?? 0;
  const needsAttention=gpsWarnings.length + openSupport;
  ...
  return <div className="min-h-screen bg-background pb-8">
    ...
    <main className="mx-auto max-w-screen-2xl space-y-6 px-4 py-5">
      <section className="overflow-hidden rounded-3xl border border-primary/20 bg-primary px-5 py-5 text-primary-foreground card-shadow sm:px-6 sm:py-6">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-amber-200">Operations control room</p><h1 className="mt-2 text-2xl font-extrabold tracking-tight sm:text-3xl">Live transport & safe intervention</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/75">Live trips, GPS health, FIFO queues and support first. Every intervention stays inside existing audited backend commands.</p></div>
          <button onClick={()=>{void loadBase();void loadQueue();}} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/10 text-white hover:bg-white/20" aria-label="Refresh operations"><RefreshCw size={16}/></button>
        </div>
      </section>

      <section className="grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4">
        <Metric label="Live cars" value={liveTrips.length} sub="Collecting + on trip" icon={<Car size={17}/>}/>
        <Metric label="GPS attention" value={gpsWarnings.length} sub="Trips needing location" icon={<MapPin size={17}/>} attention={gpsWarnings.length>0}/>
        <Metric label="Drivers waiting" value={summary?.waiting_drivers ?? 0} sub="FIFO waiting only" icon={<Users size={17}/>}/>
        <Metric label="Open support" value={openSupport} sub="Reports to review" icon={<AlertTriangle size={17}/>} attention={openSupport>0}/>
      </section>

      <section className={`rounded-2xl border px-4 py-3 ${needsAttention>0?'border-amber-200 bg-amber-50':'border-green-200 bg-green-50'}`}>
        <div className="flex items-start gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${needsAttention>0?'bg-white text-amber-700':'bg-white text-green-700'}`}><ShieldCheck size={17}/></div><div><p className="text-xs font-bold uppercase tracking-wide">Emergency boundary</p><p className="mt-1 text-sm font-bold">{needsAttention>0?`${needsAttention} operational item${needsAttention===1?'':'s'} need review`:'Guarded operations are clear'}</p><p className="mt-1 text-xs leading-relaxed opacity-80">Admin can inspect live truth and use audited queue, support, Driver and Route controls. No raw seat reassignment, FIFO bypass, verified-phone editing, fabricated GPS or active-trip Driver replacement.</p></div></div>
      </section>
      ...
      <section className="space-y-3">
        <div><p className="section-label">Live trips</p><p className="mt-1 text-xs text-muted-foreground">One operational truth across Driver, Passenger and Admin, with GPS and support context.</p></div>
        {liveTrips.length===0&&<div className="feature-card p-8 text-center text-sm text-muted-foreground">No live trips right now.</div>}
        <div className="grid gap-3 lg:grid-cols-2">{liveTrips.map(t=><div key={t.trip_id} className="overflow-hidden rounded-3xl border border-border bg-card p-4 space-y-3 card-shadow-sm sm:p-5">
          ...
          <div className="rounded-2xl border border-primary/10 bg-secondary/60 px-3 py-3"><p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Next canonical action</p><p className="mt-1 text-sm font-bold text-primary">{actionCopy[t.next_action] || t.next_action}</p>{t.next_stop_name&&<p className="mt-0.5 text-xs text-muted-foreground">{t.next_stop_name}</p>}</div>
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="section-label">Driver queue</p><p className="mt-1 text-xs text-muted-foreground">FIFO remains the default. Only WAITING Drivers can be reordered or removed; the active collector is never reassigned here.</p></div><Link href="/admin-panel/route-settings" className="text-xs font-semibold text-primary">Route controls</Link></div>
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="section-label">Driver recovery</p><p className="mt-1 text-xs text-muted-foreground">Secondary recovery only. Account deactivation remains server-blocked during a live trip.</p></div><Link href="/admin-panel/users" className="text-xs font-semibold text-primary">Open Users</Link></div>
...
function Metric({label,value,sub,icon,attention=false}:{label:string;value:number;sub:string;icon:React.ReactNode;attention?:boolean}) {
  return <div className={`rounded-2xl border bg-card p-4 card-shadow-sm ${attention?'border-amber-200':'border-border'}`}><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{label}</p><span className={attention?'text-amber-600':'text-primary'}>{icon}</span></div><p className={`mt-2 text-2xl font-extrabold ${attention?'text-amber-700':'text-foreground'}`}>{value}</p><p className="mt-1 text-[11px] text-muted-foreground">{sub}</p></div>;
}
