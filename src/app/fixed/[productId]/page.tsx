  ...
  return (
    <main className="min-h-screen bg-zinc-100 px-5 py-10 text-zinc-950"><div className="mx-auto max-w-xl">
      ...
      <section className="mt-8 rounded-3xl bg-white p-7 shadow-sm">
        <p className="text-sm font-semibold uppercase tracking-[0.14em] text-zinc-500">{product.service_type === "FIXED_ROUND_TRIP" ? "Shared round trip" : "Shared one way"}</p>
        ...
        {product.service_type === "FIXED_ROUND_TRIP" ? <p className="mt-4 rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-900">The same Driver and Vehicle stay committed for the outbound journey, destination wait and return. Your fare covers the complete shared round trip.</p> : null}
