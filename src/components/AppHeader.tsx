  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-card/95 backdrop-blur-sm">
      <div className="mx-auto flex h-16 max-w-screen-xl items-center gap-3 px-4 sm:px-6 lg:px-8">
        {showBack ? (
          <button onClick={() => router.back()} className="flex h-10 w-10 items-center justify-center rounded-xl hover:bg-muted active:scale-95" aria-label="Go back">
            <ChevronLeft size={22} />
          </button>
        ) : (
        ...
        {!showBack && (
          <nav className="hidden items-center gap-1 md:flex">
            {profile?.role === 'driver' ? (
              <Link href="/driver-route-selection" className={`nav-item ${pathname === '/driver-route-selection' ? 'active' : ''}`}><Car size={16} />Driver Home</Link>
            ) : profile?.role === 'admin' ? (
        ...
        <div className="flex items-center gap-2">
          {(profile?.role === 'passenger' || !user) && !showBack ? (
            <Link href="/" className="hidden items-center gap-1.5 rounded-xl bg-secondary px-3 py-2 text-sm font-semibold text-secondary-foreground sm:flex">
              <MapPin size={14} /><span className="max-w-[90px] truncate">{locationName}</span>
            </Link>
          ) : null}
            ...
            <div className="relative">
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-12 z-50 w-64 overflow-hidden rounded-2xl border border-border bg-card shadow-lg">
                  ...
                  <button onClick={handleSignOut} className="flex w-full items-center gap-2 px-4 py-3 text-sm text-red-600 hover:bg-red-50"><LogOut size={14} />Sign out</button>
                </div>
              )}
            </div>
          )}
