...
expect(layout,'NEXT_PUBLIC_PUBLIC_DISCOVERY_ENABLED','public discovery flag missing');
expect(robots,'disallow','robots must fail closed when discovery is disabled');
expect(robots,"'/admin-panel'",'robots must exclude private admin routes');
expect(robots,'sitemap.xml','robots must publish sitemap when discovery is enabled');
expect(sitemap,"'/outstation'",'public sitemap must include Outstation');
expect(sitemap,"'/offers'",'public sitemap must include Local Offers');
