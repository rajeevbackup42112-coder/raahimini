...
begin
  ...
  if coalesce(p_allow_contact,false) then
...
as $function$
begin
  if not public.current_user_is_admin() then return jsonb_build_object('success',false,'error','Admin access required'); end if;
...
begin
  ...
  set notification_status=p_status, notified_at=case when p_status='SENT' then now() else notified_at end, updated_at=now()
