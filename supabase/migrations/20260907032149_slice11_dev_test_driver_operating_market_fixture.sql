...
begin
  ...
  select d.id, d.home_market_id into v_driver_id, v_home_market_id
  from public.drivers d
  where d.profile_id = p_profile_id;
  if v_driver_id is null then raise exception 'TEST_DRIVER_REQUIRED'; end if;
  if v_home_market_id is null then raise exception 'TEST_DRIVER_HOME_MARKET_REQUIRED'; end if;
  ...
  insert into public.driver_operating_markets(
    driver_id, market_id, verified_at, verification_method, verification_accuracy_meters
  ) values (
    v_driver_id, v_home_market_id, now(), 'ADMIN_EXCEPTION', null
  )
  on conflict (driver_id) do update set
  ...
  return jsonb_build_object(
    ...
    'operating_market_id', v_home_market_id
  );
end;
$$;
