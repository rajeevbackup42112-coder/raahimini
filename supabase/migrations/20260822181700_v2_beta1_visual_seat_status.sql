...
begin
  ...
  select coalesce(array_agg(ts.seat_number order by ts.seat_number),array[]::integer[])
  into v_seat_numbers
  from public.trip_seats ts
  where ts.trip_id=v_request.trip_id and ts.request_id=v_request.id;
...
revoke execute on function public.get_passenger_ride_status(uuid) from public, anon, service_role;
grant execute on function public.get_passenger_ride_status(uuid) to authenticated;